...
const
  ...
  //Описание драйвера
  CNST__DRIVER_DESC = 'Testing Driver for UniFR';
...
begin
  ...
  with TIniFile.Create(ExtractFilePath(ParamStr(0)) + 'UFRTestingDrv.ini') do
  begin
...
procedure _UFRDone(Number: Integer);
var
  i, j: Integer;
begin
  FlushLogsQueue;
  for i := 0 to High(PrintDevices) do
    if PrintDevices[i].DeviceNumber = Number then
    begin
      SaveConfig(Number);
      CoUninitialize;
...
begin
  Result := False;
  AssignFile(fLog, WhereMe + 'UFRTestingDriver.log');
  if FileExists(WhereMe + 'UFRTestingDriver.log') then
    Append(fLog)
  else
    Rewrite(fLog);
  if IOResult <> 0 then
  ...
  try
    ...
    with PrintDevices[MaxLen] do
      try
        ...
        with TIniFile.Create(ExtractFilePath(ParamStr(0)) + 'UFRTestingDrv.ini') do
        begin
          LastDocNum := ReadInteger('Counters Configuration',
            'LastDocNum' + IntToStr(DevNumber), 0);
          LastReceiptNum := ReadInteger('Counters Configuration',
            'LastReceiptNum' + IntToStr(DevNumber), 0);
          LastShiftNum := ReadInteger('Counters Configuration',
            'LastShiftNum' + IntToStr(DevNumber), 0);
          SaledValue := ReadInteger('Money Counters', 'SaledValue' + IntToStr(DevNumber), 0);
          CashRegValue := ReadInteger('Money Counters', 'CashRegValue' +
...
var
  ...
  var
    ...
    Quantity, Val64, Val64_2, Sum: int64;
  begin
    Result := 0;
    Sum := 0;
    CashRes := 0;
